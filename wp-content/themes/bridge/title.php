...
if(!get_post_meta($id, "qode_show-page-title", true)) { ?>
			...
			<?php if($title_image != "" && is_page()){ ?>
				<div class="title_overlay" style="background-image:url('<?php echo $title_image; ?>');"></div>
 			<?php } ?>
	<div class="title_outer <?php echo $animate_title_class.$title_text_shadow; if($responsive_title_image == 'yes' && $show_title_image == true){ echo ' with_image'; }?>" <?php echo 'data-height="'.$title_height.'"'; if($title_height != '' && $animate_title_area == 'area_top_bottom'){ echo 'style="opacity:0;height:' . $header_height_padding .'px;"'; } ?>>
		<div class="title <?php echo $page_title_fontsize . " " . $page_title_position;
				if($responsive_title_image == 'no' && $title_image != "" && ($fixed_title_image == "yes" || $fixed_title_image == "yes_zoom") && $show_title_image == true){
					echo ' has_fixed_background ';
				if($fixed_title_image == "yes_zoom"){
					echo 'zoom_out '; } }
				if($responsive_title_image == 'no' && $title_image != "" && $fixed_title_image == "no" && $show_title_image == true){
					echo ' has_background'; }  ?>"
					style="<?php if($responsive_title_image == 'no' && $title_image != "" && $show_title_image == true){
						if($title_image_width != ''){
							/* Waves */
							/*echo 'background-size:'.$title_image_width.'px auto;';*/ }
							/*echo 'background-image:url('.$title_image.');';  */}
							if($title_height != '' && is_front_page()) {
								echo 'height:100%;';
							}
							if($title_background_color != ''){
								echo 'background-color:'.$title_background_color.';'; } ?>
									z-index:99999;">
			...
			<?php if(!get_post_meta($id, "qode_show-page-title-text", true)) { ?>
				<div class="title_holder" <?php if($responsive_title_image != 'yes' && get_post_meta($id, "qode_show-page-title-image", true) == ""){ echo $title_holder_height; }?>>
					...
					?>
						<div class="container_inner <?php if ($large_image !=  "" && is_page() ) {?>page_with_large_image <?php } ?> clearfix">
								...
								<?php } ?>
									<?php
										if(get_post_meta($id, "qode_page_subtitle", true) != "" ){
											if(get_post_meta($id, "qode_page_title_font_size", true) == "large") {
										?>
											<h4 class="subtitle <?php if (!is_page(257)) { ?>no-background<?php } ?>" <?php echo $subtitle_color; ?>><?php echo get_post_meta($id, "qode_page_subtitle", true); ?></h4>
										<?php } else { ?>
									...
									<!-- <?php if (is_front_page()) {
										wp_nav_menu( array( 'theme_location' => 'social-navigation' ,
																	'container'  => '',
																	'container_class' => '',
																	'menu_class' => '',
																	'menu_id' => '',
...
<?php
 ...
 <?php } else {
	if(!empty($title_image)) { ?>
		<img src="<?php echo $title_image; ?>" width="<?php  echo $title_image_width; ?>px" />
  <?php } else {
...
?>
<?php if (is_front_page()) { ?>
<div id="frontpage_widget">
	<div class="content_inner">
	<?php /* dynamic_sidebar( 'frontpage' ); */ ?>
	<div id="desktop-title">
	<svg class="knockout__svg_2"  xmlns="https://www.w3.org/2000/svg" xmlns:xlink="https://www.w3.org/1999/xlink" width="100%" height="100%" style="fill: rgba(255,255,255,1); top: 0; left: 0; height: 64px; margin: 0; padding: 0;">
		<mask id="text-clip-desktop" style="" height="64px"><rect id="bg-desktop" width="585px" height="100%" fill="white" style="height: 65px;"></rect>
			<text x="0.5em" y="1em" width="100%" height="100%" fill="#000" font-size="47px" font-family="'Open Sans', sans-serif">BETWEEN TWO WATERS</text>
		</mask>
		<rect width="100%" height="100%" mask="url(#text-clip-desktop)"></rect>
	</svg>
	</div>	
	<h4 class="frontpage_tagline" style="display:none;"><span>Between two waters</span></h4>
	<div id="mobile-title">
		<svg class="knockout__svg" xmlns="https://
		www.w3.org/2000/svg" xmlns:xlink="https://www.w3.org/1999/xlink" width="100%" height="100%" style="fill: rgba(255,255,255,1);top: 0;left: 0;height: 6.6em;margin: 0;padding: 0;">
		<mask id="text-clip-mobile" style="" height="124px"><rect id="bg-mobile" width="100%" height="100%" fill="white" style=""></rect>
			<text x="0.7em" y="1em" width="100%" height="100%" fill="#000" font-size="2.5em" font-family="'Open Sans', sans-serif">BETWEEN</text>
			<text x="0.7em" y="2.3em" width="100%" height="100%" fill="#000" font-size="2.5em" font-family="'Open Sans', sans-serif">TWO WATERS</text>
		</mask>
		<rect width="100%" height="100%" mask="url(#text-clip-mobile)"></rect>
	</svg>
	</div>
	<h5><span>A Galician multidisciplinary artist &amp; Visual Design Lead based in London</span></h5>
	</div>
</div>
<?php } ?>
