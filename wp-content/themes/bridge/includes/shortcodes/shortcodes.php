...
if (!function_exists('button')) {
    function button($atts, $content = null) {
        ...
        $html .=  '<a href="'.$link.'" target="'.$target.'" '.$data_attr.' class="'.$button_classes.'" style="'.$button_styles.'"><span>'.$text.'</span>'.$add_icon.'</a>';
            ...
            if($_post_format != 'quote'){
                ...
                switch ($_post_format) {
                    ...
                    case "gallery":
                        ...
                        break;
                    default:
                        if($_post_format == "link") {
                          $title_link = get_post_meta(get_the_ID(), "title_link", true) != '' ? get_post_meta(get_the_ID(), "title_link", true) : 'javascript: void(0)';
                          $html .= '<a href="' . $title_link  . '" target="_blank">';
                        } else {
                ...
                $html .= '<div class="q_masonry_blog_post_text"><div class="q_masonry_blog_post_text_frame">';

                if($_post_format == "link") {
                  $title_link = get_post_meta(get_the_ID(), "title_link", true) != '' ? get_post_meta(get_the_ID(), "title_link", true) : 'javascript: void(0)';
                  $html .= '<'.$title_tag.' class="q_masonry_blog_title"><a href="' . $title_link . '"target="_blank">' . get_the_title() . '</a></'.$title_tag.'></div>';
                } else {
                  $html .= '<'.$title_tag.' class="q_masonry_blog_title"><a href="' . get_permalink() . '">' . get_the_title() . '</a></'.$title_tag.'></div>';
                }

                $excerpt = ($text_length > 0) ? substr(get_the_excerpt(), 0, intval($text_length)) : get_the_excerpt();
                $html .= '<p class="q_masonry_blog_subtitle">'.get_post_meta(get_the_ID(), "qode_page_subtitle", true).'</p>';
                if($_post_format == "link") {
                  $title_link = get_post_meta(get_the_ID(), "title_link", true) != '' ? get_post_meta(get_the_ID(), "title_link", true) : 'javascript: void(0)';
            			$html .= '<p class="q_masonry_blog_permalink"><a href="' . $title_link . '" target="_blank">Visit</a></p>';
                } else {
                  $html .= '<p class="q_masonry_blog_permalink"><a href="' . get_permalink() . '">Read more</a></p>';
                }
                ...
                if($_post_format == "quote") {
                    $html .= '<span class="quote_author clearfix"><span class="quote_mark left">"</span><span class="quote_author_text right">' . get_the_title() . '</span></span>';
                    $html .= '<p>' . get_post_meta(get_the_ID(), "quote_format", true) . '</p>';
                } else {
    ...
    function portfolio_list($atts, $content = null) {
        ...
        if($type != 'masonry') {
            ...
            if (have_posts()) : while (have_posts()) : the_post();
                ...
                $thumb_size = $image_size == "square" ? "portfolio-square" : "full";
                ...
                $html .= "</div>";
                if ($type == "standard" || $type == "standard_no_space") {
                    $html .= "<div class='portfolio_description'". $portfolio_box_style .">";
                    $html .= '<'.$title_tag.' class="portfolio_title"><a href="' . $portfolio_link . '" target="'.$target.'">' . get_the_title() . '</a></'.$title_tag.'>';
                    $html .= '<span class="project_category">';
                    $k = 1;
                    foreach ($terms as $term) {
            ...
            if(isset($qode_options_proya["post_types_names_$post_type"])) {
                if($qode_options_proya["post_types_names_$post_type"] == $post_type) {
                    ...
                    if(isset($qode_options_proya['enable_facebook_share']) &&  $qode_options_proya['enable_facebook_share'] == "yes") {
                        ...
                        $html .= '<a href="#" onclick="window.open(\'http://www.facebook.com/sharer.php?s=100&amp;p[title]=' . qode_addslashes(get_the_title()) . '&amp;p[summary]=' . qode_addslashes(get_the_excerpt()) . '&amp;p[url]=' . urlencode(get_permalink()) . '&amp;&p[images][0]=';
        ...
        if(isset($qode_options_proya['enable_social_share']) && $qode_options_proya['enable_social_share'] == "yes") {
            ...
            if(isset($qode_options_proya["post_types_names_$post_type"])) {
                if($qode_options_proya["post_types_names_$post_type"] == $post_type) {
                    ...
                    if(isset($qode_options_proya['enable_facebook_share']) &&  $qode_options_proya['enable_facebook_share'] == "yes") {
                        $html .= '<li class="facebook_share">';
                        $html .= '<a title="'.__("Share on Facebook","qode").'" href="#" onclick="window.open(\'http://www.facebook.com/sharer.php?s=100&amp;p[title]=' . qode_addslashes(get_the_title()) . '&amp;p[summary]=' . qode_addslashes(get_the_excerpt()) . '&amp;p[url]=' . urlencode(get_permalink()) . '&amp;&p[images][0]=';
                        if(function_exists('the_post_thumbnail')) {
                            $html .=  wp_get_attachment_url(get_post_thumbnail_id());
                        }
                        $html .='\', \'sharer\', \'toolbar=0,status=0,width=620,height=280\');" href="javascript: void(0)">';
        ...
        if ($slider != "") {
            ...
            if ( have_posts() ) : $postCount = 0; while ( have_posts() ) : the_post();
				...
				if(get_post_meta(get_the_ID(), "qode_slide-hide-title", true) != true){
                    ...
                    $html_text .= '<h2 class="'.$title_classes.'" style="'.$title_color.$title_font_size.$title_line_height.$title_font_family.$title_font_style.$title_font_weight.$text_shadow.'"><span style="'.$slide_title_border_styles_string.'">'.get_the_title().'</span></h2>';
                }
