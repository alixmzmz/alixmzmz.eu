...
<body <?php body_class(); ?>>
	...
	<?php
        ...
        if (isset($qode_options_proya['enable_popup_menu'])){ if($qode_options_proya['enable_popup_menu'] == "yes") { $enable_popup_menu = "yes"; }};
...
<?php
	    ...
	    if( $large_image != "" && is_page() ){ ?>
			<div class="front_page_featured" style="background-image:url('<?php echo $large_image; ?>');"></div>
		<?php } ?>
<?php
	$header_in_grid = true;
		...
		<?php if($header_in_grid){ ?>
			...
			<?php } ?>
					<div class="header_inner_left">
						...
						<div class="logo_wrapper">
							...
							<?php
								$featured_image_array = wp_get_attachment_image_src(get_post_thumbnail_id(), $thumb_size);
        						$large_image = $featured_image_array[0]; ?>
							<?php if (is_front_page() || ($large_image != "" && is_page())) { ?>
								<img class="light" src="<?php echo $logo_image_light; ?>" alt="Logo"/>
							<?php } else { ?>
								<img class="dark" src="<?php echo $logo_image_dark; ?>" alt="Logo"/>
							<?php } ?>
                            ...
                            <div class="q_logo_hidden"><a href="<?php echo home_url('/'); ?>">
                            	<img alt="Logo" src="<?php echo $logo_image_fixed_hidden; ?>" style="height: 100%;"></a></div>
                            <?php } ?>
						</div>
					...
					<?php if($header_bottom_appearance != "stick menu_bottom"){ ?>
						...
						<?php } ?>
						<nav class="main_menu drop_down <?php if($large_image != "" && is_page() ){ echo 'featured_image_menu';} ?> <?php if($menu_position == "" && $header_bottom_appearance != "stick menu_bottom"){ echo 'right';} ?>">
						<?php
					...
					<div class="container_inner clearfix">
						...
						<div class="header_inner_left">
							...
							<div class="logo_wrapper">
								...
								if (isset($qode_options_proya['logo_image_light']) && $qode_options_proya['logo_image_light'] != ""){ $logo_image_light = $qode_options_proya['logo_image_light'];}else{ $logo_image_light =  get_template_directory_uri().'/img/logo.png'; };
								if (isset($qode_options_proya['logo_image_dark']) && $qode_options_proya['logo_image_dark'] != ""){ $logo_image_dark = $qode_options_proya['logo_image_dark'];}else{ $logo_image_dark =  get_template_directory_uri().'/img/logo_black.png'; };
								if (isset($qode_options_proya['logo_image_sticky']) && $qode_options_proya['logo_image_sticky'] != ""){ $logo_image_sticky = $qode_options_proya['logo_image_sticky'];}else{ $logo_image_§ =  get_template_directory_uri().'/img/logo_black.png'; };
                                if (isset($qode_options_proya['logo_image_popup']) && $qode_options_proya['logo_image_popup'] != ""){ $logo_image_popup = $qode_options_proya['logo_image_popup'];}else{ $logo_image_popup =  get_template_directory_uri().'/img/logo_white.png'; };
								?>
...
<?php } ?>
    ...
    <?php } ?>
		...
		<?php
			$content_class = "";
			if(get_post_meta($id, "qode_revolution-slider", true) == "" && get_post_meta($id, "qode_show-page-title", true) && !is_category() && !is_tag() && !is_author() && ($qode_options_proya['enable_google_map'] && ($header_transparency === '' || $header_transparency == 1))){
